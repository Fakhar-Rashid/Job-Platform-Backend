...
import type { LogHoursInput } from './contracts.schema.js';

export async function logHours(contractId: string, freelancerId: string, data: LogHoursInput) {
  const contract = await requirePartyContract(contractId, freelancerId);
  if (contract.freelancerId !== freelancerId) throw new HttpError(403, 'Only the freelancer can log hours');
  if (contract.status !== 'ACTIVE') throw new HttpError(409, 'This contract is not active');
  if (contract.type !== 'HOURLY') throw new HttpError(409, 'Hours only apply to hourly contracts');

  return prisma.timeEntry.create({ data: { ...data, contractId } });
}

export async function removeEntry(entryId: string, freelancerId: string) {
  ...
  if (entry.status !== 'LOGGED') throw new HttpError(409, 'Paid hours cannot be removed');
  await prisma.timeEntry.delete({ where: { id: entryId } });
}

export async function payLoggedHours(contractId: string, clientId: string) {
  return prisma.$transaction(async (tx) => {
    ...
    if (contract.clientId !== clientId) throw new HttpError(403, 'Only the client can pay hours');
    if (contract.status !== 'ACTIVE') throw new HttpError(409, 'This contract is not active');
    if (contract.type !== 'HOURLY' || contract.hourlyRate == null) {
      throw new HttpError(409, 'This is not an hourly contract');
    }
    if (contract.timeEntries.length === 0) throw new HttpError(409, 'No unpaid hours to pay');
    ...
    if (client.walletBalance < total) throw new HttpError(402, 'Not enough funds in your wallet');
