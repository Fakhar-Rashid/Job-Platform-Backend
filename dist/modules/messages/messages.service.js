...
function view(conversation, userId) {
...
export async function upsertConversation(clientId, freelancerId, jobId) {
...
export async function startConversation(clientId, { jobId, freelancerId, body }) {
    const job = await prisma.job.findUnique({ where: { id: jobId } });
    if (!job)
        throw new HttpError(404, 'Job not found');
    if (job.ownerId !== clientId)
        throw new HttpError(403, 'Only the job owner can start a conversation');
    const bid = await prisma.bid.findUnique({ where: { jobId_freelancerId: { jobId, freelancerId } } });
    if (!bid)
        throw new HttpError(409, 'You can only message freelancers who applied to this job');
...
export async function listConversations(userId, filter = 'all') {
    const rows = await loadConversations(userId);
    let views = rows.map((row) => view(row, userId));
    if (filter === 'unread')
        views = views.filter((v) => v.unread);
    if (filter === 'favorites')
...
async function activityFor(contractId) {
...
export async function setNote(id, userId, note) {
