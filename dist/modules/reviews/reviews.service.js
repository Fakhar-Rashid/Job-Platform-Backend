...
export async function createReview(jobId, authorId, { rating, comment, endorsements }) {
    ...
    if (job.ownerId !== authorId)
        throw new HttpError(403, 'Only the job owner can leave a review');
    if (job.status !== 'CLOSED')
        throw new HttpError(409, 'You can review once the job is closed');
    const hired = job.bids[0];
    if (!hired)
        throw new HttpError(409, 'This job has no hired freelancer');
    const existing = await prisma.review.findUnique({ where: { jobId } });
    if (existing)
        throw new HttpError(409, 'You have already reviewed this job');
