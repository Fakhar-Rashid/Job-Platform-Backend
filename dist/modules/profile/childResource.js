...
export function buildChildRouter({ model, schema }) {
    const router = Router();
    const delegate = prisma[model];
    async function requireOwned(id, userId) {
        const record = await delegate.findUnique({ where: { id } });
        if (!record)
            throw new HttpError(404, 'Item not found');
        if (record.userId !== userId)
            throw new HttpError(403, 'You cannot modify this item');
    }
    ...
    router.patch('/:id', validate(schema.partial()), asyncHandler(async (req, res) => {
...
//# sourceMappingURL=childResource.js.map