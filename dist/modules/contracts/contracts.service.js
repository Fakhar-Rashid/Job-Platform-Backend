...
export async function requirePartyContract(contractId, userId) {
...
export async function hire(clientId, { bidId, hourlyRate, milestones, message }) {
    const bid = await prisma.bid.findUnique({ where: { id: bidId }, include: { job: true } });
    if (!bid)
        throw new HttpError(404, 'Proposal not found');
    if (bid.job.ownerId !== clientId)
        throw new HttpError(403, 'Only the job owner can hire');
    ...
    if (existing)
        throw new HttpError(409, 'An offer or contract already exists for this proposal');
...
export async function accept(contractId, freelancerId) {
    return prisma.$transaction(async (tx) => {
        const contract = await tx.contract.findUnique({ where: { id: contractId } });
        if (!contract)
            throw new HttpError(404, 'Contract not found');
        if (contract.freelancerId !== freelancerId)
            throw new HttpError(403, 'This offer is not yours');
        if (contract.status !== 'OFFERED')
            throw new HttpError(409, 'This offer is no longer pending');
...
export async function decline(contractId, freelancerId) {
...
export async function withdraw(contractId, clientId) {
    const contract = await requirePartyContract(contractId, clientId);
    if (contract.clientId !== clientId)
        throw new HttpError(403, 'Only the client can withdraw an offer');
    if (contract.status !== 'OFFERED')
        throw new HttpError(409, 'This offer is no longer pending');
    return prisma.contract.update({ where: { id: contractId }, data: { status: 'WITHDRAWN' } });
}
export async function end(contractId, userId) {
    return prisma.$transaction(async (tx) => {
        ...
        if (contract.status !== 'ACTIVE')
            throw new HttpError(409, 'Only active contracts can be ended');
        const refundable = contract.milestones.filter((m) => ['FUNDED', 'SUBMITTED', 'CHANGES_REQUESTED'].includes(m.status));
...
export async function get(contractId, userId) {
